...
class TTSEdgeReadAloudService : BaseReadAloudService(),
    Player.Listener {
    ...
    private suspend fun getSpeakStream(
        ...
        var edge = EdgeUtil();
        edge.AssetAudioPipedStream(this)
        if (wsUrl == "" || readCount >30) {
            wsUrl = edge.getWsUrl()
            readCount = 0
        }
        readCount++
        var audioStream = edge.run(wsUrl,speakText, speechRate, "")
        return audioStream
    }
    ...
    private fun createSpeakFile(name: String, inputStream: InputStream) {
        ...
        println("createSpeakFile" + "${ttsFolderPath}$name.mp3")
